...
describe('SearchPage', () => {
    ...
    test('should render SearchPage with default values', () => {
        ...
        expect(mockSearchHeroesAction).toHaveBeenCalledWith({ name: undefined, strength: undefined });
        expect(container).toMatchSnapshot();
    });
    test('should call search action with name parameter', () => {
        const { container } = renderSearchPage(['/search?name=superman']);
        expect(mockSearchHeroesAction).toHaveBeenCalledWith({ name: 'superman', strength: undefined });
        expect(container).toMatchSnapshot();
    });
    test('should call search action with strength parameter', () => {
        const { container } = renderSearchPage(['/search?strength=6']);
        expect(mockSearchHeroesAction).toHaveBeenCalledWith({ name: undefined, strength: '6' });
        expect(container).toMatchSnapshot();
    });
    test('should call search action with strength ant name parameter', () => {
        const { container } = renderSearchPage(['/search?strength=8&name=batman']);
        expect(mockSearchHeroesAction).toHaveBeenCalledWith({ name: 'batman', strength: '8' });
        expect(container).toMatchSnapshot();
    });
    test('shoud render HeroGrid with search results', async () => {
        const mockHeroes = [
